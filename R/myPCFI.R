#' Implement parametric calibration fractional imputation
#'
...
#' @param B The number of bootstrap process (should be positive integer)
#'
...
#'
#' @examples res <- myPFI(n=200, beta=c(1, 0.7), sigma=1, phi=c(-3, 0.5, 0.7), M=10, B=2000)
#' res
myPCFI <- function(n, beta, sigma, phi, M=10, max.iter=50, eps=1e-09, B=2000){
  # Check n
  ...
  # Data generation
  devtools::source_url("https://github.com/tspark422/DoSimPFI2011/blob/main/R/Data_generation.R?raw=TRUE", sha1="8a8c6a8be122ca158f5f3a9988351cbdceb5e42d")
  ...
  for (a in 1:B){
    ...
    # Calculate initial weight w_{ij0}^{*}
    ...
    # Calculate the final fractional weights w_{ij}^{*}
    ...
    # Update parameters using final weights
    # beta
    score1 <- sum(idat_cali$weight2*(idat$y1_mis-beta_old[1]-beta_old[2]*idat$x))
    score2 <- sum(idat_cali$weight2*(idat$y1_mis-beta_old[1]-beta_old[2]*idat$x)*idat$x)
    U11 <- sum(idat_cali$weight2*(-1))
    U12 <- sum(idat_cali$weight2*(-idat$x))
    U22 <- sum(idat_cali$weight2*(-idat$x^2))
    ...
    # Sigma^2
    sigma2_new <- sum(idat_cali$weight2*(idat$y1_mis-beta_new[1]-beta_new[2]*idat$x)^2)/n
    ...
    for(l in 1:max.iter){
      ...
      score_logis_1 <- sum(idat_cali$weight2*(idat$y2_mis-pi_logis_old))
      score_logis_2 <- sum(idat_cali$weight2*(idat$y2_mis-pi_logis_old)*idat$x)
      score_logis_3 <- sum(idat_cali$weight2*(idat$y2_mis-pi_logis_old)*idat$y1_mis)
      U_logis_11 <- sum(idat_cali$weight2*pi_logis_old*(1-pi_logis_old))
      U_logis_12 <- sum(idat_cali$weight2*pi_logis_old*(1-pi_logis_old)*idat$x)
      U_logis_13 <- sum(idat_cali$weight2*pi_logis_old*(1-pi_logis_old)*idat$y1_mis)
      U_logis_22 <- sum(idat_cali$weight2*pi_logis_old*(1-pi_logis_old)*idat$x^2)
      U_logis_23 <- sum(idat_cali$weight2*pi_logis_old*(1-pi_logis_old)*idat$x*idat$y1_mis)
      U_logis_33 <- sum(idat_cali$weight2*pi_logis_old*(1-pi_logis_old)*idat$y1_mis^2)
    ...
    # Each row represents score function
    score_mean_i <- matrix(c(colSums(matrix(idat_cali$weight2*idat$score_1_ij, nrow=M)),
                             colSums(matrix(idat_cali$weight2*idat$score_2_ij, nrow=M)),
                             colSums(matrix(idat_cali$weight2*idat$score_3_ij, nrow=M)),
                             colSums(matrix(idat_cali$weight2*idat$score_4_ij, nrow=M)),
                             colSums(matrix(idat_cali$weight2*idat$score_5_ij, nrow=M)),
                             colSums(matrix(idat_cali$weight2*idat$score_6_ij, nrow=M))), nrow=6, byrow=T)
    ...
    common_term <- idat_cali$weight2*g1
    ...
    e_i_eta1 <- matrix(colSums(matrix(idat_cali$weight2*(idat$y1_mis+crossprod(K_1_eta1, score_mat)), nrow=M)), ncol=1)

    ### results for eta1 ###
    eta1 <- sum(1/n*idat_cali$weight2*idat$y1_mis)  # point estimator of eta1
    ...
    g2 <- idat$y2_mis
    common_term <- idat_cali$weight2*g2
    c1 <- sum(common_term*(idat$score_1_ij-rep(score_mean_i[1, ], each=M)))
    c2 <- sum(common_term*(idat$score_2_ij-rep(score_mean_i[2, ], each=M)))
    c3 <- sum(common_term*(idat$score_3_ij-rep(score_mean_i[3, ], each=M)))
    c4 <- sum(common_term*(idat$score_4_ij-rep(score_mean_i[4, ], each=M)))
    ...
    e_i_eta2 <- matrix(colSums(matrix(idat_cali$weight2*(idat$y2_mis+crossprod(K_1_eta2, score_mat)), nrow=M)), ncol=1)

    ### results for eta2 ###
    eta2 <- sum(1/n*idat_cali$weight2*idat$y2_mis)     # point estimator of eta2
    ...
    common_term <- idat_cali$weight2*g3
    # common_term <- idat_cali$weight2*as.numeric(Sxy/Sxx)
    c1 <- sum(common_term*(idat$score_1_ij-rep(score_mean_i[1, ], each=M)))
    c2 <- sum(common_term*(idat$score_2_ij-rep(score_mean_i[2, ], each=M)))
    c3 <- sum(common_term*(idat$score_3_ij-rep(score_mean_i[3, ], each=M)))
    c4 <- sum(common_term*(idat$score_4_ij-rep(score_mean_i[4, ], each=M)))
    ...
    e_i_eta3 <- matrix(colSums(matrix(idat_cali$weight2*(g3+crossprod(K_1_eta3, score_mat)), nrow=M)), ncol=1)

    ### results for eta3 ###
    eta3 <- sum(1/n*idat_cali$weight2*g3)     # point estimator of eta2
    ...
    common_term <- idat_cali$weight2*g4
    c1 <- sum(common_term*(idat$score_1_ij-rep(score_mean_i[1, ], each=M)))
    c2 <- sum(common_term*(idat$score_2_ij-rep(score_mean_i[2, ], each=M)))
    c3 <- sum(common_term*(idat$score_3_ij-rep(score_mean_i[3, ], each=M)))
    c4 <- sum(common_term*(idat$score_4_ij-rep(score_mean_i[4, ], each=M)))
    ...
    e_i_eta4 <- matrix(colSums(matrix(idat_cali$weight2*(as.numeric(idat$y1_mis<3)+crossprod(K_1_eta4, score_mat)), nrow=M)), ncol=1)

    ### results for eta4 ###
    eta4 <- sum(1/n*idat_cali$weight2*(idat$y1_mis<3)) # point estimator of eta4
